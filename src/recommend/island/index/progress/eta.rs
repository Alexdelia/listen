...
impl Eta {
	...
	fn left(&self, len: Option<u64>) -> Option<Duration> {
		let len = len?;
		...
		let left = u128::from(len.saturating_sub(self.done));
...
impl ProgressTracker for Eta {
	...
	fn write(&self, state: &ProgressState, w: &mut dyn fmt::Write) {
		let _ = match self.left(state.len()) {
...
mod tests {
	...
	fn what_is_left_is_what_the_finished_steps_took() {
		assert_eq!(after(60, 4).left(Some(48)), Some(Duration::from_mins(11)));
	}

	#[test]
	fn a_stage_that_finished_nothing_yet_cannot_say_when_it_ends() {
		assert_eq!(after(600, 0).left(Some(48)), None);
		assert_eq!(Eta::new().left(Some(48)), None);
	}

	#[test]
	fn a_stage_with_nothing_left_ends_now() {
		assert_eq!(after(60, 48).left(Some(48)), Some(Duration::ZERO));
	}

	#[test]
	fn a_stage_of_unknown_length_cannot_say_when_it_ends() {
		assert_eq!(after(60, 4).left(None), None);
	}
	...
	fn the_time_a_stage_spent_waiting_on_its_first_step_is_not_counted_against_it() {
		...
		assert!(hurried.left(Some(48)) < waited.left(Some(48)));
	}
}
